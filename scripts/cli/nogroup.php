...
foreach ($locs as $loc) {
    $groups = $dbhr->preQuery("SELECT * FROM `groups` WHERE (poly IS NOT NULL AND ST_Intersects(ST_GeomFromText(poly, 3857), ?)) OR (polyofficial IS NOT NULL AND ST_INTERSECTS(ST_GeomFromText(polyofficial, 3857), ?)) LIMIT 1;", [
        $loc['geometry'],
        $loc['geometry']
    ]);
