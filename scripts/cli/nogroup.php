...
foreach ($locs as $loc) {
    $groups = $dbhr->preQuery("SELECT * FROM `groups` WHERE (poly IS NOT NULL AND ST_Intersects(ST_GeomFromText(poly), ?)) OR (polyofficial IS NOT NULL AND ST_INTERSECTS(ST_GeomFromText(polyofficial), ?)) LIMIT 1;", [
        $loc['geometry'],
        $loc['geometry']
    ]);
