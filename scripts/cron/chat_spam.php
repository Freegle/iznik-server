...
$chats = $dbhr->preQuery("SELECT chat_rooms.id, userid, user1, user2 FROM `chat_rooms` 
    INNER JOIN spam_users ON user1 = userid WHERE latestmessage >= '$mysqltime' AND flaggedspam = 0 AND collection = ? 
    UNION 
    SELECT chat_rooms.id, userid, user1, user2 FROM `chat_rooms` 
    INNER JOIN spam_users ON user2 = userid WHERE latestmessage >= '$mysqltime' AND flaggedspam = 0 AND collection = ?;", [
    Spam::TYPE_SPAMMER,
    Spam::TYPE_SPAMMER
]);
...
foreach ($chats as $chat) {
    ...
    $messages = $dbhr->preQuery("SELECT COUNT(*) AS count FROM chat_messages WHERE chatid = ? AND reviewrequired = 0 AND reviewrejected = 0 AND processingsuccessful = 1;", [
        $chat['id']
    ]);

    if ($messages[0]['count'] > 0) {
        error_log("Chat with spammer users {$chat['user1']} {$chat['user2']} chat id {$chat['id']}");
        ...
        if (!Mail::ourDomain($innocent->getEmailPreferred()) && strpos($innocent->getEmailPreferred(), GROUP_DOMAIN) === FALSE) {
            error_log("...sending warning to {$innocent->getEmailPreferred()}");
            ...
            Mail::addHeaders($dbhr, $dbhm, $message, Mail::SPAM_WARNING, $innocent->getId());
...
$users = $dbhr->preQuery("SELECT DISTINCT chat_messages.userid, COUNT(*) AS count 
    FROM chat_messages 
    LEFT JOIN spam_users ON spam_users.userid = chat_messages.userid 
    INNER JOIN users ON users.id = chat_messages.userid 
    WHERE
    chat_messages.date >= ? AND
    reviewrejected = 1 AND (collection IS NULL OR collection != 'Whitelisted') AND systemrole = 'User' 
    GROUP BY chat_messages.userid HAVING count > 5 ORDER BY count DESC;", [ $start ]);
$count = 0;
