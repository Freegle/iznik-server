        ...
        if (ourDomain($envto) && $g->onYahoo()) {
            ...
            if ($uid) {
                ...
                if ($cont) {
                    ...
                    $msgs = $dbhr->preQuery("SELECT messages_groups.msgid FROM messages_groups LEFT JOIN messages_outcomes ON messages_outcomes.msgid = messages_groups.msgid INNER JOIN messages ON messages.id = messages_groups.msgid WHERE fromuser = ? AND groupid = ? AND collection = ? AND messages_outcomes.msgid IS NULL;", [
                        $uid,
                        $gid,
