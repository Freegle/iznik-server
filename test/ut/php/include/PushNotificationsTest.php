...
class PushNotificationsTest extends IznikTestCase {
    ...
    public function testBasic() {
        ...
        # Build debug info for assertion message - this ensures it's captured in test output on failure.
        $debugInfo = "notifyGroupMods returned $notifyCount. " .
            "groupid={$this->groupid}, mod=$id, sender=$id2, room=$rid, msg=$cm. " .
            "Mods: " . json_encode($this->dbhm->preQuery("SELECT userid, role FROM memberships WHERE groupid = ? AND role IN ('Owner', 'Moderator')", [$this->groupid])) . ". " .
            "MT Push: " . json_encode($this->dbhm->preQuery("SELECT id, type, apptype FROM users_push_notifications WHERE userid = ? AND apptype = ?", [$id, PushNotifications::APPTYPE_MODTOOLS])) . ". " .
            "Roster: " . json_encode($this->dbhm->preQuery("SELECT chatid, userid, lastmsgnotified FROM chat_roster WHERE chatid = ?", [$rid]));

        $this->assertEquals(1, $notifyCount, $debugInfo);
