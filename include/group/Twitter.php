...
class Twitter {
    ...
    public function tweetStory($id = NULL) {
        ...
        # - a random one
        $idq = $id ? " AND id = $id " : "";
        $sql = "SELECT id FROM users_stories WHERE public = 1 AND newsletterreviewed = 1 AND DATEDIFF(NOW(), date) <= 30 $idq ORDER BY tweeted ASC, RAND() LIMIT 1;";
    ...
    WHERE messages_groups.groupid = ? AND
          (messages_groups.arrival >= ? OR (messages_groups.approvedat IS NOT NULL AND messages_groups.approvedat >= ?)) AND 
          messages_groups.collection = ? AND 
          messages.type IN (?, ?) AND
