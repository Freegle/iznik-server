...
class Twitter {
    ...
    public function tweetStory($id = NULL) {
        ...
        # - a random one
        $idq = $id ? " AND id = $id " : "";
        $sql = "SELECT id FROM users_stories WHERE public = 1 AND newsletterreviewed = 1 AND newsletter = 0 $idq ORDER BY tweeted ASC LIMIT 1;";
    ...
    public function tweetMessages() {
        # We want to tweet any messages since the last one, with a max to avoid flooding things.
