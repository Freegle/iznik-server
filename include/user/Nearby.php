...
class Nearby
{
    ...
    public function messages($groupid) {
        ...
        if ($g->getSetting('relevant', 1)) {
            ...
            $sql = "SELECT DISTINCT messages_spatial.msgid AS id, messages_spatial.msgtype AS type, ST_Y(messages_spatial.point) AS lat, ST_X(messages_spatial.point) AS lng, messages.fromuser, messages.subject 
FROM messages_spatial INNER JOIN messages ON messages_spatial.msgid = messages.id
INNER JOIN `groups` ON groups.id = messages_spatial.groupid
WHERE groupid = ? AND messages.fromuser IS NOT NULL AND messages_spatial.successful = 0 AND listable = 1 AND onmap = 1;";
