...
;", [
    ...
    public function getPublicHistory($me, &$rets, $users, $groupids, $historyfull, $systemrole, $msgcoll = [ MessageCollection::APPROVED ]) {
        ...
        if ($groupids && count($groupids) > 0) {
            ...
            $sql = "SELECT GREATEST(COALESCE(messages_postings.date, messages.arrival), COALESCE(messages_postings.date, messages.arrival)) AS postdate, messages_outcomes.outcome, messages.fromuser, messages.id, messages.fromaddr, messages.arrival, messages.date, messages_groups.collection, messages_postings.date AS repostdate, messages_postings.repost, messages_postings.autorepost, messages.subject, messages.type, DATEDIFF(NOW(), messages.date) AS daysago, messages_groups.groupid FROM messages INNER JOIN messages_groups ON messages.id = messages_groups.msgid AND groupid IN ($groupq) $collq AND fromuser IN (" . implode(',', $userids) . ") AND messages_groups.deleted = 0 LEFT JOIN messages_postings ON messages.id = messages_postings.msgid LEFT JOIN messages_outcomes ON messages_outcomes.msgid = messages.id HAVING postdate > ? ORDER BY postdate DESC;";
        } else if ($systemrole == User::SYSTEMROLE_SUPPORT || $systemrole == User::SYSTEMROLE_ADMIN) {
            # We can see all groups.
            $sql = "SELECT GREATEST(COALESCE(messages_postings.date, messages.arrival), COALESCE(messages_postings.date, messages.arrival)) AS postdate, messages_outcomes.outcome, messages.fromuser, messages.id, messages.fromaddr, messages.arrival, messages.date, messages_groups.collection, messages_postings.date AS repostdate, messages_postings.repost, messages_postings.autorepost, messages.subject, messages.type, DATEDIFF(NOW(), messages.date) AS daysago, messages_groups.groupid FROM messages INNER JOIN messages_groups ON messages.id = messages_groups.msgid $collq AND fromuser IN (" . implode(',', $userids) . ") AND messages_groups.deleted = 0 LEFT JOIN messages_postings ON messages.id = messages_postings.msgid LEFT JOIN messages_outcomes ON messages_outcomes.msgid = messages.id HAVING postdate > ? ORDER BY postdate DESC;";
        }
