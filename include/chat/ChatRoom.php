        ...
        if (!$this->id) {
            ...
            if ($userid == $this->chatroom['user1'] || $userid == $this->chatroom['user2']) {
                ...
                if (!$cansee) {
                    ...
                    #error_log("CanSee $userid, {$this->id}, " . var_export($rooms, TRUE) . " gives $cansee");
                }
            }
...
ORDER BY chat_messages.id DESC LIMIT 1;", [
    ...
    public function referToSupport() {
        ...
        $message = \Swift_Message::newInstance()
            ->setSubject($me->getName() . " (#" . $me->getId() . ") asked for help with chat #{$this->id} with " . $this->getName($this->id, $me->getId()) . " (#" . $this->getPrivate('user1') . ")")
