...
ORDER BY lastdate DESC;";
    ...
    public function addToSpatialIndex() {
        if ($this->lng || $this->lat) {
            ...
            foreach ($groups as $g) {
                ...
                $sql = "INSERT INTO messages_spatial (msgid, point, groupid, msgtype, arrival) VALUES (?, ST_GeomFromText('POINT({$this->lng} {$this->lat})', 3857), ?, ?, ?) ON DUPLICATE KEY UPDATE point = ST_GeomFromText('POINT({$this->lng} {$this->lat})', 3857), groupid = ?, msgtype = ?, arrival = ?;";
                $this->dbhm->preExec(
                    $sql,
                    [
                        $this->id,
    ...
    public function searchActiveInBounds($string, $messagetype, $swlat, $swlng, $nelat, $nelng, $groupid = NULL, $exactonly = FALSE) {
        ...
        if (!$groupid) {
            $sql = "SELECT id FROM `groups` WHERE ST_Intersects(polyindex, ST_GeomFromText('$poly', 3857)) AND onmap = 1 AND publish = 1;";
            $groups = $this->dbhr->preQuery($sql);
...
$mq", [
    ...
    WHERE messages_groups.arrival >= ? AND messages.lat IS NOT NULL AND messages.lng IS NOT NULL AND messages.deleted IS NULL AND messages_groups.collection = ? AND 
        ...
        foreach ($msgs as $msg) {
            $sql = "INSERT INTO messages_spatial (msgid, point, groupid, msgtype, arrival) VALUES (?, ST_GeomFromText('POINT({$msg['lng']} {$msg['lat']})', 3857), ?, ?, ?) ON DUPLICATE KEY UPDATE point = ST_GeomFromText('POINT({$msg['lng']} {$msg['lat']})', 3857), groupid = ?, msgtype = ?, arrival = ?;";
            $this->dbhm->preExec($sql, [
                $msg['id'],
                $msg['groupid'],
                $msg['msgtype'],
